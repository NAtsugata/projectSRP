...
// Ce fichier regroupe de nombreuses icônes SVG et composants d'interface
// partagés dans l'application. La version ci‑dessous reprend le code
// original du dépôt `projectSRP` et y applique un correctif au niveau de la
// détection du device dans le composant CustomFileInput. La logique
// d'origine utilisait `navigator.userAgent` directement, ce qui peut
// provoquer des erreurs sur mobile et dans des environnements SSR où
// `navigator` n'est pas défini. Nous introduisons donc une variable
// `ua` protégée pour éviter ces exceptions tout en conservant le même
// comportement.
...
export const CustomFileInput = ({ onChange, accept, multiple, disabled, children, className = '' }) => {
  ...
  // Détection des capacités du device avec garde pour navigator
