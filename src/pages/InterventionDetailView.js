...
const SignatureModal = ({ onSave, onCancel, existingSignature }) => {
  ...
  useEffect(() => {
    ...
    return ()=>{ canvas.removeEventListener('mousedown',start); canvas.removeEventListener('mouseup',stop); canvas.removeEventListener('mousemove',draw);
      canvas.removeEventListener('mouseleave',stop); canvas.removeEventListener('touchstart',start); canvas.removeEventListener('touchend',stop); canvas.removeEventListener('touchmove',draw); };
  }, [existingSignature]);
  return (
    <div className="modal-overlay"><div className="modal-content signature-modal-content">
      ...
      <div className="modal-footer" style={{marginTop:'1rem'}}>
        <button type="button" onClick={()=>{const c=canvasRef.current;if(c){c.getContext('2d').clearRect(0,0,c.width,c.height);}}}
                className="btn btn-secondary">Effacer</button>
        <button type="button" onClick={onCancel} className="btn btn-secondary">Annuler</button>
        <button type="button" onClick={()=>onSave(canvasRef.current.toDataURL('image/png'))}
                className="btn btn-primary" disabled={!hasDrawn}>Valider</button>
      </div>
    </div></div>
  );
};
...
const InlineUploader = ({
  ...
  const compressImageIfNeeded = useCallback(async(file)=>{
    ...
    if (file.size < 512 * 1024) return file; // < 500KB : pas de transfo
    return new Promise((resolve)=>{
      ...
      img.onload = () => {
        ...
        canvas.toBlob((blob)=>{
          ...
          resolve(blob ? new File([blob], file.name.replace(/\.(heic|heif)$/i, '.jpg'), { type:'image/jpeg', lastModified:Date.now() }) : file);
        }, 'image/jpeg', 0.82);
      };
  ...
  const doUpload = useCallback(async(files)=>{
    ...
    for (let i=0;i<files.length;i++){
      try{
        ...
        if (!publicUrl) throw new Error('URL publique non retournée.');
...
export default function InterventionDetailView({
  ...
  isAdmin: _isAdmin, dataVersion: _dataVersion, refreshData: _refreshData // <- éviter no-unused-vars en CI
}) {
  ...
  }, [interventions, interventionId, navigate, ensureReportSchema]);
  ...
  // ---- Check & validation de clôture (déclarée AVANT usage -> pas de no-use-before-define)
    ...
    <div className="intervention-detail">
      ...
      <div className="quick-action-bar">
        <button className="qa-btn" onClick={openStartValidation} disabled={!!report.arrivalTime}>
          <CheckCircle2Icon/>{' '}
          {report.arrivalTime ? 'Arrivée enregistrée' : 'Arrivé'}
        </button>
      ...
      <div className="card-white">
        <div className="header-row">
          ...
          <div className="status-badges">
            <span className="badge">Statut : {intervention.status || (report.arrivalTime ? 'En cours' : 'Planifiée')}</span>
          ...
          <div style={{marginTop:8}}>
            <VoiceNoteRecorder
              ...
              onUploaded={async(uploaded)=>{
                const updated={...report, files:[...(report.files||[]), ...uploaded.map(u=>({...u, tag:'Documents'}))]};
                await persistReport(updated);
              }}
            />
          </div>
                ...
                <li key={n.id}>
                  <div style={{flexGrow:1}}>
                    <p className="font-semibold">
                      [{n.category||'—'}] {n.label}{n.qty?` × ${n.qty}`:''}
            ...
            <div className="table-like">
              ...
              {report.parts_used.map((p,idx)=> (
                <div className="trow" key={`${p.ref}-${idx}`}>
                  <div>{p.ref||'—'}</div><div>{p.label||'—'}</div><div>{p.qty||1}</div><div>{p.serial||'—'}</div>
          ...
          <div className="grid" style={{gridTemplateColumns:'1fr 1fr', gap:'1rem'}}>
            {blockKeys.map(({key,label})=> (
              <div key={key} className="card-sub">
                <label className="checkbox">
                  <input type="checkbox" checked={!!report.blocks?.[key]?.blocked}
                         onChange={(e)=>setBlockField(key,'blocked', e.target.checked)} /> <b>{label}</b>
                </label>
                ...
                {Array.isArray(report.blocks?.[key]?.photos) && report.blocks[key].photos.length>0 && (
                  <div className="thumbs">
                    {report.blocks[key].photos.map((ph,idx)=> (
                      <OptimizedImage key={idx} src={ph.url} alt={ph.name} className="thumb"
        ...
        <div className="section">
          ...
          <InlineUploader
            interventionId={interventionId}
            onUploadComplete={async(uploaded)=>{
              const updated={...report, files:[...(report.files||[]), ...uploaded.map(u=>({...u, tag: activeMediaTag}))]};
              await persistReport(updated);
            }}
          />
        </div>
        ...
        <div className="section">
          ...
          {report.signature ? (
            <div>
              <img src={report.signature} alt="Signature"
                   style={{width:'100%',maxWidth:300,border:'2px solid #e5e7eb',borderRadius:'0.5rem',background:'#f8f9fa'}}/>
              <button onClick={()=>persistReport({ ...report, signature: null })} className="btn btn-sm btn-secondary" style={{marginTop:8}}>
                Effacer
              </button>
            </div>
          ) : (
            <div>
              ...
              <div style={{marginTop:8}}>
                <button onClick={()=>setShowSignatureModal(true)} className="btn btn-secondary"><ExpandIcon/> Agrandir</button>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="section">
          <button onClick={openCloseValidation} disabled={isSaving}
                  className="btn btn-primary w-full" style={{fontSize:'1rem',padding:'1rem',fontWeight:600}}>
      ...
      {cancelModal.open && (
        <div className="modal-overlay"><div className="modal-content">
          ...
          <textarea className="form-control" rows={3} value={cancelModal.reason}
                    onChange={(e)=>setCancelModal(m=>({...m, reason: e.target.value}))} placeholder="Motif obligatoire…" />
          <div className="grid" style={{gridTemplateColumns:'1fr 1fr', gap:'0.75rem', marginTop:'0.5rem'}}>
            <div>
              ...
              <input className="form-control" value={cancelModal.initials}
                     onChange={(e)=>setCancelModal(m=>({...m, initials: e.target.value}))} placeholder="ex: AB"/>
            </div>
            <div style={{display:'flex',alignItems:'flex-end'}}>
              <label className="checkbox"><input type="checkbox" checked={cancelModal.agree}
                                                 onChange={(e)=>setCancelModal(m=>({...m, agree: e.target.checked}))}/> <span>Je confirme l'annulation</span></label>
            </div>
          </div>
          <div className="modal-footer">
            ...
            <button className="btn btn-danger" onClick={confirmCancel}
                    disabled={!cancelModal.reason.trim() || !cancelModal.initials.trim() || !cancelModal.agree}>Confirmer</button>
          </div>
        </div></div>
      )}
        ...
        <div className="modal-overlay"><div className="modal-content">
          ...
          <div className="grid" style={{gridTemplateColumns:'1fr 1fr', gap:'0.75rem'}}>
            <div>
              ...
              <input className="form-control" value={closeModal.initials}
                     onChange={(e)=>setCloseModal(m=>({...m, initials: e.target.value}))} placeholder="ex: AB"/>
            </div>
            <div style={{display:'flex',alignItems:'flex-end'}}>
              <label className="checkbox"><input type="checkbox" checked={closeModal.agree}
                                                 onChange={(e)=>setCloseModal(m=>({...m, agree: e.target.checked}))}/> <span>Je confirme la clôture</span></label>
            </div>
          </div>
          <div className="modal-footer">
            ...
            <button className="btn btn-primary" onClick={confirmClose}
                    disabled={!closeModal.initials.trim() || !closeModal.agree}>Clôturer</button>
          </div>
        </div></div>
      )}
      ...
      {startModal.open && (
        <div className="modal-overlay"><div className="modal-content">
          ...
          <div className="grid" style={{gridTemplateColumns:'1fr 1fr', gap:'0.75rem'}}>
            <div>
              <label>Initiales technicien</label>
              <input className="form-control" value={startModal.initials}
                     onChange={(e)=>setStartModal(m=>({...m, initials: e.target.value}))} placeholder="ex: AB"/>
            </div>
            <div style={{display:'flex',alignItems:'flex-end'}}>
              <label className="checkbox"><input type="checkbox" checked={startModal.agree}
                                                 onChange={(e)=>setStartModal(m=>({...m, agree: e.target.checked}))}/> <span>Je confirme mon arrivée</span></label>
            </div>
          </div>
          <div className="modal-footer">
            ...
            <button className="btn btn-primary" onClick={confirmStart}
                    disabled={!startModal.initials.trim() || !startModal.agree}>Valider</button>
          </div>
        </div></div>
      )}
      ...
      {resumeModal.open && (
        <div className="modal-overlay"><div className="modal-content">
          ...
          <div className="grid" style={{gridTemplateColumns:'1fr 1fr', gap:'0.75rem'}}>
            <div>
              <label>Initiales technicien</label>
              <input className="form-control" value={resumeModal.initials}
                     onChange={(e)=>setResumeModal(m=>({...m, initials: e.target.value}))} placeholder="ex: AB"/>
            </div>
            <div style={{display:'flex',alignItems:'flex-end'}}>
              <label className="checkbox"><input type="checkbox" checked={resumeModal.agree}
                                                 onChange={(e)=>setResumeModal(m=>({...m, agree: e.target.checked}))}/> <span>Je confirme la reprise</span></label>
            </div>
          </div>
          <div className="modal-footer">
            <button className="btn btn-secondary" onClick={()=>setResumeModal({ open:false, initials:'', agree:false })}>Annuler</button>
            <button className="btn btn-primary" onClick={confirmResume}
                    disabled={!resumeModal.initials.trim() || !resumeModal.agree}>Valider</button>
          </div>
        </div></div>
      )}
    </div>
...
// -------- Enregistreur de note vocale (fallback si Safari) --------
