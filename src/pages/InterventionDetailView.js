...
export default function AdminArchiveView({ showToast, showConfirmationModal }) {
  ...
  const fetchArchived = useCallback(async () => {
    ...
    if (error) { showToast("Erreur de chargement des archives.", "error"); } else { setArchived(data || []); }
    setLoading(false);
  }, [showToast]);
  ...
  const handleDeleteArchive = (id) => {
    showConfirmationModal({
      ...
      message:"Action irréversible (intervention + fichiers associés).",
      onConfirm: async ()=>{
        const { error } = await interventionService.deleteIntervention(id);
        if (error) showToast("Erreur lors de la suppression.", "error"); else { showToast("Archive supprimée avec succès."); fetchArchived(); }
      }
    });
  };
  ...
  const handleExport = async (intervention) => {
    ...
    const extractUrlFromFileItem = (item)=>{ if(!item) return null; if(typeof item==='string') return item; return item.url || item.file_url || item.path || null; };
    const looksLikeImage = (u)=> typeof u==='string' && (u.startsWith('data:image/') || /\.(png|jpe?g|webp|gif|bmp|tiff?)($|\?)/i.test(u));
    ...
    try {
      ...
      const status = intervention.status || (intervention.report?.arrivalTime ? 'En cours' : 'À venir');
      let y = 34; doc.text('Statut: ' + status, 10, y); y += 7;
      if (status === 'Annulée' && intervention.cancellation_reason) { const c = doc.splitTextToSize('Motif annulation: ' + intervention.cancellation_reason, 180); doc.text(c, 10, y); y += c.length * 6 + 2; }
      doc.text('Notes:', 10, y); y += 6; const notes = doc.splitTextToSize(intervention.report?.notes || 'Aucune', 180); doc.text(notes, 10, y); y += notes.length * 6 + 2;
      if (Array.isArray(intervention.report?.needs) && intervention.report.needs.length) { if (y > 250) { doc.addPage(); y = 10; } doc.text('Besoins chantier:', 10, y); y += 6; const rows = intervention.report.needs.map(n => `• ${n.label}${n.qty ? ' × ' + n.qty : ''}${n.urgent ? ' [URGENT]' : ''}${n.note ? ' — ' + n.note : ''}`); for (const r of rows) { const t = doc.splitTextToSize(r, 180); if (y + t.length * 6 > 280) { doc.addPage(); y = 10; } doc.text(t, 10, y); y += t.length * 6; } }
      if (intervention.report?.signature) { if (y > 200) { doc.addPage(); y = 10; } doc.text('Signature du client', 10, y); if (isDataUrl(intervention.report.signature)) { const sigMime = (intervention.report.signature.split(';')[0] || 'image/png').replace('data:', ''); const fmt = /jpeg|jpg/i.test(sigMime) ? 'JPEG' : 'PNG'; doc.addImage(intervention.report.signature, fmt, 10, y + 5, 180, 80); } else { doc.text('(signature jointe dans le ZIP)', 10, y + 90); } }
      ...
      const reportImageUrls = Array.isArray(intervention?.report?.files) ? intervention.report.files.map(extractUrlFromFileItem).filter(u=>u && looksLikeImage(u)) : [];
      ...
      if (employeePhotos.length) {
        for (const imageItem of employeePhotos) {
          const url = extractUrlFromFileItem(imageItem); if(!url || !looksLikeImage(url)) continue;
