// =============================
// FILE: src/pages/InterventionDetailView.js — FULL (fix ESLint + validations technicien + uploader robuste)
// =============================
...
// ✅ Résout l’URL publique quelle que soit la forme renvoyée
const resolvePublicUrl = (res) => {
  const candidates = [
  ...
  const found = candidates.find((v) => typeof v === 'string' && /^https?:\/\//.test(v));
  return found || null;
};
  ...
  useEffect(() => {
    ...
    if ('IntersectionObserver' in window && imgRef.current) {
      const obs = new IntersectionObserver(
        (entries) => { if (entries[0].isIntersecting) { img.src = src; obs.disconnect(); } },
        { rootMargin: '50px' }
      );
...
const SignatureModal = ({ onSave, onCancel, existingSignature }) => {
  ...
  useEffect(() => {
    ...
    return () => {
      canvas.removeEventListener('mousedown', start); canvas.removeEventListener('mouseup', stop); canvas.removeEventListener('mousemove', draw); canvas.removeEventListener('mouseleave', stop);
      canvas.removeEventListener('touchstart', start); canvas.removeEventListener('touchend', stop); canvas.removeEventListener('touchmove', draw);
    };
  }, [existingSignature]);
  return (
...
// -------- Uploader mobile/desktop robuste --------
const InlineUploader = ({
    ...
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        canvas.toBlob((blob) => {
          ...
          resolve(
            blob
              ? new File([blob], file.name.replace(/\.(heic|heif)$/i, '.jpg'), { type: 'image/jpeg', lastModified: Date.now() })
              : file
          );
        }, 'image/jpeg', 0.82);
      };
  ...
  const doUpload = useCallback(async (files) => {
    ...
    setState({ uploading: true, queue: queued, error: null });

    const uploaded = [];
    for (let i = 0; i < files.length; i++) {
      try {
        const f = await compressImageIfNeeded(files[i]);
        const result = await storageService.uploadInterventionFile(f, interventionId, folder, (p) => {
          setState((s) => ({
            ...s,
            queue: s.queue.map((it, idx) => (idx === i ? { ...it, status: 'uploading', progress: p } : it)),
          }));
        });
        if (result?.error) throw result.error;
        const publicUrl = resolvePublicUrl(result);
        if (!publicUrl) throw new Error('URL publique non retournée par le service de stockage.');
        uploaded.push({ name: files[i].name, url: publicUrl, type: files[i].type });
        setState((s) => ({
          ...s,
          queue: s.queue.map((it, idx) => (idx === i ? { ...it, status: 'completed', progress: 100 } : it)),
        }));
      } catch (err) {
        setState((s) => ({
          ...s,
          queue: s.queue.map((it, idx) => (idx === i ? { ...it, status: 'error', error: String(err?.message || err) } : it)),
        }));
      }
    }
    if (uploaded.length) {
      try { await onUploadComplete(uploaded); }
      catch { setState((s) => ({ ...s, error: 'La sauvegarde des fichiers a échoué.' })); }
    }
    setState((s) => ({ ...s, uploading: false }));
  }, [compressImageIfNeeded, interventionId, folder, onUploadComplete]);

  const onPickFromLibrary = useCallback((e) => {
    const files = Array.from(e.target.files || []);
    if (fileInputRef.current) fileInputRef.current.value = '';
    doUpload(files);
  }, [doUpload]);

  const onTakePhoto = useCallback((e) => {
    const files = Array.from(e.target.files || []);
  ...
  return (
    <div className="mobile-uploader-panel">
      ...
      <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
        <button onClick={() => cameraInputRef.current?.click()} className={`btn btn-secondary w-full flex-center ${state.uploading ? 'disabled' : ''}`} disabled={state.uploading}>
          📷 Prendre une photo
        </button>
        <button onClick={() => fileInputRef.current?.click()} className={`btn btn-secondary w-full flex-center ${state.uploading ? 'disabled' : ''}`} disabled={state.uploading}>
...
export default function InterventionDetailView({
  interventions, onSave, onSaveSilent, onUpdateStatus,
  isAdmin: _isAdmin, dataVersion, refreshData: _refreshData // éviter no-unused-vars
}) {
    ...
    return {
      ...
      parts_used: Array.isArray(r.parts_used)
        ? r.parts_used.map((p) => ({
            ...
            qty: Number(p.qty) || 1,
            serial: p.serial || '',
            unit_price: numberOrNull(p.unit_price),
          }))
        : [],
  ...
  // ---- Validation de clôture (déclarée AVANT usage pour ESLint) ----
  function validateCanClose() {
    ...
    const anyBlocked = ['access', 'power', 'parts', 'authorization'].some((k) => report?.blocks?.[k]?.blocked);
  ...
  const needsTotal = useMemo(
    () => (Array.isArray(report?.needs) ? report.needs.reduce((sum, n) => sum + (Number(n.estimated_price) || 0), 0) : 0),
    [report?.needs]
  );
  const addNeed = async () => {
  ...
  const partsTotal = useMemo(
    () => (Array.isArray(report?.parts_used) ? report.parts_used.reduce((s, p) => s + (Number(p.unit_price) || 0) * (Number(p.qty) || 1), 0) : 0),
    [report?.parts_used]
  );
  const addPart = async () => {
    if (!partDraft.ref && !partDraft.label) return;
    const item = {
      ref: partDraft.ref.trim(),
      label: partDraft.label.trim(),
      qty: Math.max(1, Number(partDraft.qty) || 1),
      serial: partDraft.serial.trim(),
      unit_price: numberOrNull(partDraft.unit_price),
    };
  ...
  const filteredFiles = useMemo(
    () => (report?.files || []).filter((f) => (activeMediaTag ? (f.tag || 'Documents') === activeMediaTag : true)),
    [report?.files, activeMediaTag]
  );
  ...
  return (
    <div className="intervention-detail">
      ...
      <div className="quick-action-bar">
        <button className="qa-btn" onClick={openStartValidation} disabled={!!report.arrivalTime}>
          <CheckCircle2Icon /> {report.arrivalTime ? 'Arrivée enregistrée' : 'Arrivé'}
        </button>
        <button className="qa-btn" onClick={() => setActiveMediaTag('En cours')}>
          <CameraIcon /> Photo
        </button>
        <button className="qa-btn" onClick={() => document.getElementById('need-input')?.focus()}>
          ➕ Besoin
        </button>
        <button className="qa-btn" onClick={() => document.getElementById('part-ref')?.focus()}>
          🔧 Pièce
        </button>
        <button className="qa-btn" onClick={() => setActiveMediaTag('Anomalies')}>
          ⚠️ Blocage
        </button>
        {currentStatus !== 'Annulée' ? (
          <button className="qa-btn danger" onClick={openCancel}>
            <XCircleIcon /> Annuler
          </button>
        ) : (
          <button className="qa-btn" onClick={openResumeValidation}>
            <RefreshCwIcon /> Reprise
          </button>
        )}
        <button className="qa-btn primary" onClick={openCloseValidation} disabled={isSaving}>
          <PaperPlaneIcon /> Clôturer
        </button>
      </div>
      ...
      <div className="card-white">
        ...
        <div className="section">
          ...
          <textarea
            value={report.notes || ''}
            onChange={(e) => persistReport({ ...report, notes: e.target.value })}
            placeholder="Détails, observations, travaux effectués…"
            rows={5}
          ...
          <div style={{ marginTop: 8 }}>
            <VoiceNoteRecorder
              ...
              onUploaded={async (uploaded) => {
                const updated = {
                  ...report,
                  files: [...(report.files || []), ...uploaded.map((u) => ({ ...u, tag: 'Documents' }))],
                };
                await persistReport(updated);
              }}
            />
            ...
            {report.quick_checkpoints.map((c, idx) => (
              <li key={idx}>
                ...
                <small className="text-muted">
                  {c.at ? new Date(c.at).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }) : '-'}
                </small>
              </li>
            ))}
          </ul>
                ...
                <li key={n.id}>
                  <div style={{ flexGrow: 1 }}>
                    ...
                    <p className="text-muted" style={{ fontSize: '0.875rem' }}>
                      {n.note || '—'}{' '}
                      {typeof n.estimated_price === 'number' ? ` • Estimé: ${n.estimated_price.toFixed(2)} €` : ''}
                    </p>
                  </div>
          ...
          <div className="grid" style={{ gridTemplateColumns: '160px 80px 120px 1fr 140px auto', gap: '0.5rem', alignItems: 'end' }}>
            <div>
              ...
              <select
                ...
                onChange={(e) => setNeedDraft((v) => ({ ...v, category: e.target.value }))}
              >
            ...
            <div>
              ...
              <input
                ...
                className="form-control"
                value={needDraft.qty}
                onChange={(e) => setNeedDraft((v) => ({ ...v, qty: Math.max(1, Number(e.target.value) || 1) }))}
              />
            </div>
            <div>
              <label>Urgent ?</label>
              <select
                className="form-control"
                value={needDraft.urgent ? '1' : '0'}
                onChange={(e) => setNeedDraft((v) => ({ ...v, urgent: e.target.value === '1' }))}
              >
            ...
            <div style={{ gridColumn: '1 / -1' }}>
              ...
              <input
                ...
                onChange={(e) => setNeedDraft((v) => ({ ...v, label: e.target.value }))}
                placeholder="Ex: Tuyau 16mm"
              />
            </div>
            <div>
              ...
              <input
                className="form-control"
                value={needDraft.estimated_price}
                onChange={(e) => setNeedDraft((v) => ({ ...v, estimated_price: e.target.value }))}
            ...
            <div>
              ...
              <input
                className="form-control"
                value={needDraft.note}
                onChange={(e) => setNeedDraft((v) => ({ ...v, note: e.target.value }))}
                placeholder="Détail, lien, réf…"
              />
            </div>
            <div style={{ gridColumn: '1 / -1' }}>
        ...
        <div className="section">
          <div className="section-header">
            ...
            <div className="text-muted">Total: <b>{partsTotal.toFixed(2)} €</b></div>
          </div>
          ...
          {Array.isArray(report.parts_used) && report.parts_used.length > 0 && (
            <div className="table-like">
              <div className="trow thead">
                <div>Réf</div><div>Désignation</div><div>Qté</div><div>N° série</div><div>Prix unitaire (€)</div><div></div>
              </div>
          ...
          <div className="grid" style={{ gridTemplateColumns: '140px 1fr 80px 160px 160px auto', gap: '0.5rem', alignItems: 'end' }}>
            <div>
              ...
              <input
                ...
                className="form-control"
                value={partDraft.ref}
                onChange={(e) => setPartDraft((v) => ({ ...v, ref: e.target.value }))}
            ...
            <div>
              ...
              <input
                className="form-control"
                value={partDraft.label}
                onChange={(e) => setPartDraft((v) => ({ ...v, label: e.target.value }))}
                placeholder="Ex: Pompe 40W"
              />
            </div>
            <div>
              <label>Qté</label>
              <input
                ...
                className="form-control"
                value={partDraft.qty}
                onChange={(e) => setPartDraft((v) => ({ ...v, qty: Math.max(1, Number(e.target.value) || 1) }))}
              />
            </div>
            <div>
              <label>N° série (opt)</label>
              <input
                className="form-control"
                value={partDraft.serial}
                onChange={(e) => setPartDraft((v) => ({ ...v, serial: e.target.value }))}
                placeholder="SN-..."
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => setPartDraft((v) => ({ ...v, unit_price: e.target.value }))}
                placeholder="ex: 12.90"
              />
            </div>
            <div>
              <button className="btn btn-secondary" onClick={addPart} disabled={!partDraft.ref && !partDraft.label}>
                Ajouter
              </button>
            </div>
          </div>
            ...
            {blockKeys.map(({ key, label }) => (
              <div key={key} className="card-sub">
                <label className="checkbox">
                  <input
                    type="checkbox"
                    checked={!!report.blocks?.[key]?.blocked}
                    onChange={(e) => setBlockField(key, 'blocked', e.target.checked)}
                  />{' '}
                  <b>{label}</b>
                </label>
                <textarea
                  ...
                  value={report.blocks?.[key]?.note || ''}
                  onChange={(e) => setBlockField(key, 'note', e.target.value)}
                />
                <div className="mt-2">
                  <InlineUploader
                    interventionId={interventionId}
        ...
        <div className="section">
          <div className="section-header">
            ...
            <div className="tabs">
              {MEDIA_TAGS.map((tag) => (
                <button
                  key={tag}
                  className={`tab ${activeMediaTag === tag ? 'active' : ''}`}
                  onClick={() => setActiveMediaTag(tag)}
                >
                  {tag}
                </button>
              ))}
                ...
                return (
                  <li key={`${file.url || idx}-${idx}`} className="document-item-optimized">
                    {file.type?.startsWith('image/') ? (
                      ...
                      <div style={{ width: 40 }}><audio controls src={file.url} style={{ height: 32 }} /></div>
                    ) : (
                    ...
                    <select
                      className="form-control"
                      value={file.tag || 'Documents'}
                      onChange={(e) => moveFileTag(originalIdx, e.target.value)}
                    >
                      {MEDIA_TAGS.map((t) => (
          ...
          <InlineUploader
            ...
            onUploadComplete={async (uploaded) => {
              const updated = {
                ...report,
                files: [...(report.files || []), ...uploaded.map((u) => ({ ...u, tag: activeMediaTag }))],
              };
              await persistReport(updated);
            }}
          />
        ...
        <div className="section">
          ...
          {report.signature ? (
            <div>
              ...
              />
              <button
                onClick={() => persistReport({ ...report, signature: null })}
            ...
            <div>
              <canvas
                width="300"
                height="150"
                style={{ border: '2px dashed #cbd5e1', borderRadius: '0.5rem', width: '100%', maxWidth: 300, background: '#f8fafc' }}
              />
              <div style={{ marginTop: 8 }}>
        ...
        <div className="section">
          <button
            onClick={openCloseValidation}
            disabled={isSaving}
            className="btn btn-primary w-full"
            style={{ fontSize: '1rem', padding: '1rem', fontWeight: 600 }}
          >
      ...
      {showSignatureModal && (
        <SignatureModal
          onSave={(sig) => { persistReport({ ...report, signature: sig }); setShowSignatureModal(false); }}
        ...
        <div className="modal-overlay"><div className="modal-content">
          ...
          <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginTop: '0.5rem' }}>
            <div>
              ...
              <input
                className="form-control"
                value={cancelModal.initials}
                onChange={(e) => setCancelModal((m) => ({ ...m, initials: e.target.value }))}
                placeholder="ex: AB"
              />
            </div>
            <div style={{ display: 'flex', alignItems: 'flex-end' }}>
              <label className="checkbox">
                <input
                  type="checkbox"
                  checked={cancelModal.agree}
                  onChange={(e) => setCancelModal((m) => ({ ...m, agree: e.target.checked }))}
                />{' '}
                <span>Je confirme l'annulation</span>
              </label>
            </div>
          </div>
          <div className="modal-footer">
            <button className="btn btn-secondary" onClick={() => setCancelModal({ open: false, reason: '', initials: '', agree: false })}>
              Fermer
            </button>
            <button
              className="btn btn-danger"
              onClick={confirmCancel}
              disabled={!cancelModal.reason.trim() || !cancelModal.initials.trim() || !cancelModal.agree}
            >
              Confirmer
            </button>
          </div>
        ...
        <div className="modal-overlay"><div className="modal-content">
          ...
          <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
            <div>
              ...
              <input
                className="form-control"
                value={closeModal.initials}
                onChange={(e) => setCloseModal((m) => ({ ...m, initials: e.target.value }))}
                placeholder="ex: AB"
              />
            </div>
            <div style={{ display: 'flex', alignItems: 'flex-end' }}>
              <label className="checkbox">
                <input
                  type="checkbox"
                  checked={closeModal.agree}
                  onChange={(e) => setCloseModal((m) => ({ ...m, agree: e.target.checked }))}
                />{' '}
                <span>Je confirme la clôture</span>
              </label>
            </div>
          </div>
          <div className="modal-footer">
            <button className="btn btn-secondary" onClick={() => setCloseModal({ open: false, initials: '', agree: false })}>
              Annuler
            </button>
            <button
              className="btn btn-primary"
              onClick={confirmClose}
              disabled={!closeModal.initials.trim() || !closeModal.agree}
            >
              Clôturer
            </button>
          </div>
        ...
        <div className="modal-overlay"><div className="modal-content">
          ...
          <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
            <div>
              ...
              <input
                className="form-control"
                value={startModal.initials}
                onChange={(e) => setStartModal((m) => ({ ...m, initials: e.target.value }))}
                placeholder="ex: AB"
              />
            </div>
            <div style={{ display: 'flex', alignItems: 'flex-end' }}>
              <label className="checkbox">
                <input
                  type="checkbox"
                  checked={startModal.agree}
                  onChange={(e) => setStartModal((m) => ({ ...m, agree: e.target.checked }))}
                />{' '}
                <span>Je confirme mon arrivée</span>
              </label>
            </div>
          </div>
          <div className="modal-footer">
            <button className="btn btn-secondary" onClick={() => setStartModal({ open: false, initials: '', agree: false })}>
              Annuler
            </button>
            <button
              className="btn btn-primary"
              onClick={confirmStart}
              disabled={!startModal.initials.trim() || !startModal.agree}
            >
              Valider
            </button>
          </div>
        ...
        <div className="modal-overlay"><div className="modal-content">
          ...
          <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
            <div>
              ...
              <input
                className="form-control"
                value={resumeModal.initials}
                onChange={(e) => setResumeModal((m) => ({ ...m, initials: e.target.value }))}
                placeholder="ex: AB"
              />
            </div>
            <div style={{ display: 'flex', alignItems: 'flex-end' }}>
              <label className="checkbox">
                <input
                  type="checkbox"
                  checked={resumeModal.agree}
                  onChange={(e) => setResumeModal((m) => ({ ...m, agree: e.target.checked }))}
                />{' '}
                <span>Je confirme la reprise</span>
              </label>
            </div>
          </div>
          <div className="modal-footer">
            <button className="btn btn-secondary" onClick={() => setResumeModal({ open: false, initials: '', agree: false })}>
              Annuler
            </button>
            <button
              className="btn btn-primary"
              onClick={confirmResume}
              disabled={!resumeModal.initials.trim() || !resumeModal.agree}
            >
              Valider
            </button>
          </div>
...
// -------- Enregistreur de note vocale (fallback iOS si besoin) --------
function VoiceNoteRecorder({ onUploaded, interventionId }) {
  ...
  const start = async () => {
    try {
      ...
      const mediaRec = new MediaRecorder(stream, mimeType ? { mimeType } : undefined);
      chunksRef.current = [];
      ...
      mediaRec.onstop = async () => {
        ...
        const publicUrl = resolvePublicUrl(res);
        if (!publicUrl) { alert("Impossible de récupérer l'URL publique de la note audio."); return; }
  ...
  return (
    <div className="flex items-center gap-2">
      {!recording ? (
        <button className="btn btn-secondary" onClick={start}><MicIcon /> Enregistrer une note</button>
      ) : (
