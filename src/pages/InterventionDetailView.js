...
import { ChevronLeftIcon, DownloadIcon, FileTextIcon, LoaderIcon, ExpandIcon, RefreshCwIcon, XCircleIcon, CheckCircleIcon, AlertTriangleIcon, MicIcon, StopCircleIcon, PaperPlaneIcon, CheckCircle2Icon } from '../components/SharedUI';
...
export default function InterventionDetailView({ interventions, onSave, onSaveSilent, isAdmin, dataVersion, refreshData }) {
  ...
  // Harmonise/assure le schéma du report pour éviter les crash si champs absents
  ...
  // ⚠️ FIX ICI: on persiste toujours SOUS report côté backend
  const persistReport = async (updated, { refresh=true } = {}) => {
    setReport(updated); // maj UI immédiate
    try {
      // *** IMPORTANT: payload attendu par le backend ***
      await onSaveSilent(intervention.id, { report: updated });
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      // ⚠️ Sauvegarde finale: conserve la convention { report: ... }
      await onSave(intervention.id, { report: report, admin_notes: adminNotes });
    }
    ...
    <div>
      ...
      <div className="card-white">
        ...
        <div className="section">
          ...
          <textarea value={report.notes||''} onChange={e=>handleReportChange('notes', e.target.value)} placeholder="Détails, matériel, observations..." rows="5" className="form-control"/>
          <VoiceNoteRecorder
