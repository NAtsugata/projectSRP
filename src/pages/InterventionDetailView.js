...
// (Expérience Technicien + validations + uploader mobile robuste)
// =============================
...
// ✅ Résout l’URL publique quelle que soit la forme renvoyée par storageService
...
const InlineUploader = ({
  ...
  // Compression image si “grosse” (sinon on garde l’original)
  const compressImageIfNeeded = useCallback(async (file) => {
    if (!file?.type?.startsWith('image/')) return file;
    if (file.size < 512 * 1024) return file; // < 500KB
...
export default function InterventionDetailView({
  interventions, onSave, onSaveSilent, onUpdateStatus, isAdmin, dataVersion, refreshData
}) {
  ...
  // validations technicien (initiales + consentement)
      ...
      {closeModal.open && (
        <div className="modal-overlay"><div className="modal-content">
          ...
          <div className="modal-footer">
            ...
            <button className="btn btn-primary" onClick={confirmClose} disabled={!closeModal.initials.trim() || !closeModal.agree}>
              Clôturer
            </button>
          </div>
        </div></div>
      ...
      {startModal.open && (
        <div className="modal-overlay"><div className="modal-content">
          ...
          <div className="modal-footer">
            ...
            <button className="btn btn-primary" onClick={confirmStart} disabled={!startModal.initials.trim() || !startModal.agree}>
              Valider
            </button>
          </div>
        </div></div>
      ...
      {resumeModal.open && (
        <div className="modal-overlay"><div className="modal-content">
          ...
          <div className="modal-footer">
            ...
            <button className="btn btn-primary" onClick={confirmResume} disabled={!resumeModal.initials.trim() || !resumeModal.agree}>
              Valider
            </button>
          </div>
        </div></div>
...
function VoiceNoteRecorder({ onUploaded, interventionId }) {
  ...
  const start = async () => {
    try {
      ...
      mediaRec.onstop = async () => {
        ...
        if (!publicUrl) {
          alert("Impossible de récupérer l'URL publique de la note audio.");
          return;
        }
  ...
  const stop = () => {
    try { rec?.stop(); setRecording(false); } catch (e) {}
  };
