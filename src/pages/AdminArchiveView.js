...
export default function AdminArchiveView({ showToast, showConfirmationModal }) {
  ...
  const fetchArchived = useCallback(async () => {
    ...
    if (error) { showToast("Erreur de chargement des archives.", "error"); }
  ...
  const handleDeleteArchive = (id) => {
    showConfirmationModal({
      ...
      onConfirm: async ()=>{
        ...
        if (error) { showToast("Erreur lors de la suppression de l'archive.", "error"); }
        else { showToast("Archive supprimée avec succès."); fetchArchived(); }
      }
    });
  };
