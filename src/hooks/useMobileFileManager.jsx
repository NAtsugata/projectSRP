...
export const useMobileFileManager = (interventionId) => {
  ...
  const handleFileUpload = useCallback(async (files, onComplete) => {
    ...
    try {
      ...
      // ⚠️ CRUCIAL : On laisse le temps à l'UI de s'afficher avant de bloquer avec la compression
      // Cela permet à l'utilisateur de voir la file d'attente "pending" immédiatement
      setTimeout(async () => {
          ...
          for (let i = 0; i < validFiles.length; i += concurrentUploads) {
            ...
            const batchPromises = batch.map(async (file, idx) => {
              ...
              try {
                ...
                // Petit délai pour laisser l'UI respirer entre chaque compression
