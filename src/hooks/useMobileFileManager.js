// src/hooks/useMobileFileManager.js - Hook optimisé pour la gestion des fichiers mobile avec détection sécurisée
...
export const useMobileFileManager = (interventionId) => {
  ...
  // ✅ DÉTECTION DEVICE OPTIMISÉE AVEC GARDE
  ...
  if (!deviceInfoRef.current) {
    ...
    const connectionType =
      isNavigator && navigator.connection && navigator.connection.effectiveType
    ...
    async (files, onComplete) => {
      ...
      try {
        ...
        for (const file of Array.from(files)) {
          const maxSize =
            deviceInfo.connectionType === '2g' ? 5 * 1024 * 1024 : 10 * 1024 * 1024;
          ...
          if (
            ...
            invalidFiles.push({
              file,
              reason: file.size > maxSize ? 'Fichier trop volumineux' : 'Type non supporté'
            });
          }
        }
